...
const {getIp}= require('./localip.js');
...
class Util {
  ...
  static deviceConfig = [
    ...
    },
    {
      ...
      "battery": false,
      "class": "socket",
      "capabilities_1": ["onoff", "measure_power", "meter_power", "meter_power.returned", "measure_voltage", "measure_current", "measure_temperature", "input_1", "input_2", "rssi", "button.enable_ble_script", "button.disable_ble_script", "actionEvents", "deviceGen2", "multiInputs"],
      "capabilities_2": [],
      "capability_options": {
        "meter_power.returned": {"title": {"en": "Energy Returned", "nl": "Retour energie"}},
        "measure_temperature": {"title": {"en": "Device Temperature", "nl": "Apparaat temperatuur"}},
    ...
    },
    {
      ...
      "capability_options": {
        ...
        "measure_temperature": {"title": {"en": "Device Temperature RGB", "nl": "Apparaat temperatuur RGB"}},
        "measure_temperature.light": {"title": {"en": "Device Temperature CCT", "nl": "Apparaat temperatuur CCT"}},
    ...
    },
    {
      ...
      "capability_options": {
        ...
        "measure_temperature": {"title": {"en": "Device Temperature RGB", "nl": "Apparaat temperatuur RGB"}},
        "measure_temperature.light": {"title": {"en": "Device Temperature Light", "nl": "Apparaat temperatuur Licht"}},
        "dim": {"opts": {"duration": true}, "title": {"en": "Dim Level RGB", "nl": "Dim niveau RGB"}},
 ...
 if (BLEConfig.enable === false) {
    ...
    try {
      ...
      for (const driver of drivers) {
        ...
        for (const device of devices) {
          if (purpose === 'collection' && device.getStoreValue('communication') !== 'zwave') { /* device collection excluding zwave devices */
  ...
  async getVirtualComponents(address, password, filter_type) {
    try {
      ...
      const virtual_components_data = await this.sendRPCCommand('/rpc/Shelly.GetComponents?dynamic_only=true&include=["config"]', address, password);
  ...
  async enableBLEProxy(address, password) {
    try {
      ...
      await this.sendRPCCommand('/rpc/Script.SetConfig?id='+ scriptCreated.result.id +'&config={"enable":true}', address, password);
